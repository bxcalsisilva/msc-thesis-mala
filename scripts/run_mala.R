...
SCENARIO_ID <- sprintf("mala_n%d_p%d_rho%s", N_OBS, P_TOTAL, rho_label)
...
for (i in 1:N_SIM) {
  ...
  # Ejecución MALA
  res <- run_mala_logit_parallel(mala_data, params_mala, sim_seeds[i])
  ...
  if (!is.null(res$resumen)) {
    ...
    # Cálculo de métricas de error y cobertura
    ...
    # Exportación de cadenas
    saveRDS(res$muestras, 
            file = sprintf("output/chains/mala/%s_iter_%03d.rds", SCENARIO_ID, i), 
            compress = "xz")
  }
